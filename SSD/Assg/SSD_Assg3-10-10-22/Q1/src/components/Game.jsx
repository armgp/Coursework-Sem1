...
export default function Game() {
  ...
  useEffect(() => {
    ...
    game.scene.add(startBoard.board);
    startBoard.animateBoard();
    ...
    const onMouseClick = (event) => {
      ...
      if (!gameStarted) {
        ...
        game.scene.add(downloadEvents.board);
      } else {
        mouse.x = (event.clientX / window.innerWidth) * 2 - 1;
        mouse.y = -(event.clientY / window.innerHeight) * 2 + 1;
        raycaster.setFromCamera(mouse, game.camera);
        const intersects = raycaster.intersectObjects(
          downloadEvents.board.children
        );
        if (intersects.length > 1) exportMouseInfo();
      }
    };
    ...
    const controls = new DragControls(
      kboard.crowsAndVulture.children,
      game.camera,
    ...
    controls.addEventListener("dragstart", (event) => {
      ...
      i++;
      if (
        (crowsTurn && event.object.player == "crow") ||
        (!crowsTurn && event.object.player == "vulture")
      ) {
    ...
    const onMouseUp = (event) => {
      if (i != undefined) mouseEvents.push([`MouseEvents${i}`, "mouse up"]);
      i++;
      mouse.x = (event.clientX / window.innerWidth) * 2 - 1;
      mouse.y = -(event.clientY / window.innerHeight) * 2 + 1;
      raycaster.setFromCamera(mouse, game.camera);
      const intersects = raycaster.intersectObjects(
        kboard.hiddenTiles.children
      );
      if (intersects.length > 0 && !intersects[0].object.isOccupied) {
        ...
        if (currDraggedObj.player == "vulture" && nextMoves.length != 0)
          nextMoves = nextMoves.slice(-1 * n, -2);
      ...
      if (!crowsTurn && vulturePlayer.nextMoves.length != 0) {
        ...
        if (didCrowsWin) {
          ...
          game.scene.remove(playerBoard.board);
          var wb = new WinnerBoard("CROWS");
          game.scene.add(wb.board);
          wb.animateBoard();
        ...
        if (vultureKills == 4) {
          ...
          game.scene.remove(game.scene.children[4]);
          game.scene.remove(playerBoard.board);
          var wb = new WinnerBoard("VULTURE");
          game.scene.add(wb.board);
          wb.animateBoard();
          window.addEventListener(
            "mouseup",
